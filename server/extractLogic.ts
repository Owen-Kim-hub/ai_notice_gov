  ...
  if (skipped.length > 0) {
    ...
    parts.push(
      `수집 시간이 예산을 초과하여 우선순위 하위 포털(${list})은 이번 검색에서 수집되지 않았습니다. ` +
        `위 결과는 그 이전 단계까지 수집된 것입니다. ` +
        `[해결 방법] 상기 포털에서 연결이 지연된 경우입니다. 잠시 후 다시 접속해 주십시오. ` +
        `만일 수집 대상 기간을 default에서 변경하신 경우는 "수집 대상 시작일"을 "종료일"에 더 가깝게(검색 기간을 짧게) 설정한 뒤 다시 추출하면, ` +
        `포털당 처리량이 줄어 누락된 포털까지 모두 수집됩니다. 일반적으로는 30일이내는 무리없이 수집됩니다.`
    );
  }
